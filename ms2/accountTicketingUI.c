...
int menuLogin(struct AccountTicketingData* pAccTicData) {
	...
	if (num == 0) {
		...
		for (j = 2; j >-1; j--) {//
			...
			printf("User Login        : ");//the number of space is 9 different with pdf
			scanf("%s", username);
			printf("Password          : ");
			scanf("%s", password);
	...
	for (;;) {
			...
			if (accountnumber == pAcc->accountNumber) {
				...
				if (iFinded != -1) {
					...
					if (option == 'Y' || option == 'y') {						
						...
						pAccTickData->accounts[iFinded].accountType = 0;//remove all information about account,11.18
						...
						printf("\n*** Account Removed! (%d ticket(s) removed) ***\n\n", iCount);//11.18 display the number of tickets deleted
						pauseExecution();
					}
				}
				else {
		...
		case 5:
			...
			for (m = 0; m < pAccTickData->TICKET_MAX_SIZE; m++)
			{
				...
				if (pAccTickData->tickets[m].ticketState == 1 && pAccTickData->tickets[m].ticketCounter == 1) {
					printf("%06d %05d %-15s %5s %-25s %10d", pAccTickData->tickets[m].ticketNumber,
						pAccTickData->tickets[m].accountNumber, pAccTickData->tickets[m].messageType->displayName,//maybe need index for messageType, messageType[0].
				...
				for (n = 0; n < pAccTickData->TICKET_MAX_SIZE; n++) {
					if (choice == pAccTickData->tickets[n].ticketNumber) {
						if (pAccTickData->tickets[n].ticketState == 1) {
							//printf("%d", pAccTickData->tickets[n].ticketCounter);//ticketnumber must be in the closed lists
							printf("\n================================================================================\n");
							printf("%06d (%6s) Re: %-25s", pAccTickData->tickets[n].ticketNumber, arrTicketState, pAccTickData->tickets[n].subjectLine);
							printf("\n================================================================================\n");
							int ii = 0;
							for (ii = 0; ii < 20; ii++) {
		...
		case 8:
			...
			if (iEmptyIndex == -1) {
				...
				//getTicket(middleParam, pAccTickData);
